...
/*
  ...
  Porque "localhost" no celular significa o próprio celular.

  Você deverá colocar o IP do seu computador na rede.

  Exemplo:

  http://192.168.0.15:8000

  Para descobrir no Windows:
...
function extractErrorMessage(
  ...
  if (
    ...
    /*
      FastAPI também pode retornar erros
      de validação como array:
...
Promise<Language[]> {
  ...
  const response =
    await request<LanguagesResponse>(
      "/languages",
      {
        method: "GET",
      }
...
export async function generateAudio(
  ...
  return request<GenerateAudioResponse>(
    "/audio",
    {
      method: "POST",
    ...
    // Áudio pode demorar um pouco mais
    60000
  );
...
translateAndGenerateAudio(
  ...
  return request<TranslateAudioResponse>(
    "/translate-audio",
    {
      method: "POST",
